...
import static io.anuke.mindustry.Vars.d2registry;
...
public class CustomBlock{
    ...
    public void checkUnlink(Tile powerTile1, Tile powerTile2){ // unlink tiles if they are linked
        ...
        try{
            if(powerTile1.entity().power.links.contains(powerTile2.pos()))
                powerTile1.configureAny(powerTile2.pos());
            else if(powerTile2.entity().power.links.contains(powerTile1.pos()))
                powerTile1.configureAny(powerTile2.pos());
        }
        catch(Exception e) {
            e.printStackTrace();
    ...
    public float powerBalance(Tile tile){
        //return tile.entity().power.graph.getPowerProduced()-tile.entity().power.graph.getPowerNeeded();
    ...
    public void digitalWrite(int write, Tile t, Tile t2[]){
        for (int i = 0; i < t2.length; i++){
            if((write & 1) > 0) checkLink(t2[i], t);
            else checkUnlink(t2[i], t);
            write = write >> 1;
        }
    }
    ...
    public class Component {
        ...
        public boolean checkBuild(){ //create a block if it doesn't exist, return false if not valid
            if (!tile.block().equals(block)) {
                ...
                Call.onConstructFinish(tile, block , 0, (byte)0, team, false);
            }
            return true;
        }
    }
