package io.anuke.mindustry.world.logic.CustomBlock;
...
class Diode extends CustomBlock {
    ...
    void flip(Tile batt1, Tile batt2){
        checkUnlink(node2,batt1);
        checkLink(node2,batt2);
        checkUnlink(node,batt2);
        checkLink(node,batt1);
    }
}
...
class SorterDisplay16 extends CustomBlock {
    ...
    void sorterWrite(int i, int state){
        int current = ((SorterEntity)sorters[i].entity()).config();//content.items().indexOf(((SorterEntity)sorters[i].entity()).config());
        if (current != state) sorters[i].configure((Math.max(Math.min(state,15),0)));//Call.setSorterItem(null, sorters[i], content.items().get(Math.max(Math.min(state,15),0)));
    }
}
...
class SorterDisplayD2 extends CustomBlock {
    ...
    class D2 extends DeskSquared{
        ...
        int getData(){
            ...
            return ((SorterEntity)sorters[pos++].entity()).config();//content.items().indexOf(((SorterEntity)sorters[pos++].entity()).sortItem);
        }
        @Override
        void setData(int d){
            if (d<0){
