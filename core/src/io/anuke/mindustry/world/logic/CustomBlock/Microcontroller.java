...
public class Microcontroller extends CustomBlock {
    ...
    @Override
    void logic(){
        //digitalWrite(true, solar, vcc);
        //solar.configureAny(1);
        //vcc.configureAny(solar.pos());
        //System.out.println(vcc.block().name + " " + solar.block().name);
        digitalWrite(true, vcc,solar);
    ...
    class Interpreter {
        ...
        void step() {
            ...
            if (s.length >= 2){
                if(s[1].contains("portc")) digitalWrite(vals.get("portc"), pins, vcc);
                else if (s[1].contains("porta")) digitalWrite(vals.get("porta"), porta, vcc);
                else if (s[1].contains("portb")) digitalWrite(vals.get("portb"), portb, vcc);
            }
        }
